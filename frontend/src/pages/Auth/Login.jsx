...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.email === 'admin@eapprovals.gov' || form.email === 'demo@investor.in') {
      const user = { id: 1, name: 'Rajesh Kumar', email: form.email, role: form.email.includes('gov') ? 'government' : 'investor' }
      login(user)
      navigate('/dashboard')
    } else {
      // Accept any valid-looking email for demo
      const user = { id: 1, name: form.email.split('@')[0], email: form.email, role: 'investor' }
      login(user)
      navigate('/dashboard')
    }
    setLoading(false)
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500 mt-1">Sign in to your approval portal</p>
          </div>
          ...
          <div className="mt-4 p-3 rounded-lg bg-slate-50 border border-slate-100">
            <p className="text-xs text-slate-400 text-center">
              Demo: Use any email to sign in, or try <code className="bg-slate-200 px-1 rounded">admin@eapprovals.gov</code> for government role
            </p>
          </div>
        </div>
      </div>
